function Tag(impl, conf, innerHTML) {
  ...
  this.mount = function() {
    ...
    if (!self.parent || self.parent.mounted) {
      self.mounted = true
      self.trigger('mount')
    }
    // otherwise we need to wait that the parent event gets triggered
      ...
      if (!isInStub(self.root)) {
        self.parent.mounted = self.mounted = true
        self.trigger('mount')
      }
    })
  }
  ...
  function toggle(isMount) {
    ...
    each(childTags, function(child) {
      console.log(root, 'is mounting ', child.root, ' who is in stub? ', root.inStub)
